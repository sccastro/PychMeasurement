...
`r getSeason(as.POSIXct(Sys.Date()))`, `r strsplit(as.character(Sys.Date()),split = "-")[[1]][1]`
...
clssdays <- function(classday,Semester = "Spring", Year = "2024") {
  if (Semester == "Spring"){
    allclass <- seq(ymd(paste0(Year,'-01-17')),ymd(paste0(Year,'-05-06')), by = 'day')
    holidays <- c(ymd(paste0(Year,'-01-15')),
                  ymd(paste0(Year,'-01-21')),ymd(paste0(Year,'-02-19')),seq.Date(from = ymd(paste0(Year,'-03-25')),to = ymd(paste0(Year,'-03-28')),by = 'day'))
    Dates <- format(allclass[classday == weekdays(allclass) & allclass %ni% holidays],"%b %d")
    dfdate <- data.frame(Dates)
    return(dfdate)
  } else {
    allclass <- seq(ymd(paste0(Year,'-08-26')),ymd(paste0(Year,'-12-11')), by = 'day')
    holidays <- c(ymd(paste0(Year,'-09-07')),ymd(paste0(Year,'-11-11')),
                  seq.Date(from = ymd(paste0(Year,'-11-25')),to = ymd(paste0(Year,'-11-27')),by = 'day'))
    Dates <- format(allclass[classday == weekdays(allclass) & allclass %ni% holidays],"%b %d")
    dfdate <- data.frame(Dates)
    return(dfdate)
  }
...
urls <- c("https://github.com/rstudio/cheatsheets/blob/main/base-r.pdf",
...
schedule <- cbind(clssdays(classday = "Monday",Semester = "Spring",Year = "2024"),
      "Topic/Description" = c("Introduction/Installation","R commands/tricks","Intro to reading data/Data Workshop","Cleaning data/Data Workshop", "Dplyr and Tidyr","Visualizing with ggplot/Data
Workshop","Intro to stats in R","More stats in R","Still more stats in R","Rmarkdown","Report Generation and Optimization","Rmarkdown help",
"Bonus Material – Shiny?","Course Review"),
      "Reading/Work" = c("syllabus",
                         paste0("Swirl tutorial/[base-R](",urls[1],")"),
                         ...
                         paste0("Stats challenge/[Navarro](",urls[6],")"),
                         paste0("Stats challenge/[MLM tutorial](",urls[7],")"),
                         paste0("Stats challenge/[MLM tutorial](",urls[8],")"),
                         paste0("Report challenge/[Rmarkdown-2.0](",urls[9],")"),
                         paste0("Report challenge/[rmarkdown-reference](",urls[10],")"),
                         paste0("Shiny app build/[shiny](",urls[11],")"),
                         paste0("Regular expression challenge/[purrr](",urls[12],")","/[strings](",urls[13],")"),
                         paste0("Presentations of Data Analysis")),
      "Project Milesone/task" = c("","Obtaining a Dataset","Ask your first question on StackOverflow","Dataset Read into R",
                                  ...
                                  "Methods & Results Due","Final Projects Due"))
